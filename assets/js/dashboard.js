async function initDashboard() {
  ...
  if (alertList) {
    ...
    alertList.innerHTML = rows.length ? rows.map(r => {
      ...
      return `<div class="alert-row"><div class="alert-main"><div class="row-title">${APP.escape(r.employee_name)}｜${APP.escape(r.license_name)}</div><div class="row-meta">${APP.escape(r.center || '')}　期限：${APP.fmtDate(r.expiration_date)}</div></div><div class="alert-days">${APP.badge(days < 0 ? `${Math.abs(days)}日超過` : `残り${days}日`, cls)}</div></div>`;
