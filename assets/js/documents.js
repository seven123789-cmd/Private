...
window.EmployeeDocuments = (() => {
  ...
  function documentTypeOptions(entityType) {
    ...
    if (entityType === 'employee_employment_contract') {
      return [{value:'雇用・契約書類', label:'雇用・契約書類'}];
    }
  ...
  function configureUploadContext() {
    ...
    const isEmploymentContract = type === 'employee_employment_contract';

    if (title) {
      title.textContent =
        type === 'employee_license' ? '資格・免許の書類を追加' :
        isEmploymentContract ? '雇用契約の書類を追加' :
    ...
    if (subtitle) {
      subtitle.textContent =
        ...
        isEmploymentContract ? 'この雇用契約に直接紐づけて保存します' :
